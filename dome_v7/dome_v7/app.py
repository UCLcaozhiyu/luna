...
def handle_set_ready(data):
    ...
    if room in rooms:
        ...
        if username in room_data['players']:
            ...
            # ✅ 检查是否至少一个人已 ready，若是就停止水波动画
            if any(player['ready'] for player in room_data['players'].values()):
                led_controller.idle_animation_active = False  # 停止水波动画
...
def end_game(room):
    print("[end_game]游戏结束")
    room_data = rooms[room]
    led_controller.idle_animation_active = True  # 恢复水波动画
